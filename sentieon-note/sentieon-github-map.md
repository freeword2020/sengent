...
- 公开仓库总数页面显示为 `18`
- 热门仓库里最值得关注的是 `sentieon-scripts`、`sentieon-models`、`sentieon-cli`
...
- 问“怎么用 / 参数什么意思 / 限制是什么”时，优先文档站
- 问“有没有现成脚本 / 模型去哪拿 / 容器怎么起 / 云上怎么部署”时，优先 GitHub
...
- 官方单命令 pipeline CLI
- 适合标准化执行 `DNAscope`、`DNAscope LongRead`、`DNAscope Hybrid`、`Pangenome`
...
- 真正找当前模型，优先 `sentieon-models`
- 真正找当前单命令流程，优先 `sentieon-cli`
...
- 这类仓库更适合历史参考
- 当前实践优先 `sentieon-cli`、`sentieon-models`、文档站
...
- 标准 pipeline 优先看 `sentieon-cli`
- 模型清单优先看 `sentieon-models`
- 示例脚本看 `sentieon-scripts`
